...
const TerminalComponent = ({ closeTerminal }) => {
  ...
  useEffect(() => {
    ...
    // document.querySelector('.xterm').addEventListener('input', e => {
    ...
    function runFakeTerminal() {
      ...
      term.prompt()
      document.querySelector('.xterm').addEventListener('keyup', e => {
        e.preventDefault()
        e.stopPropagation()
        e.cancelBubble = true
        console.log(e)
    ...
    const commands = {
      show: {
        ...
        description: '[project name]: show particular project'
      },
      projects: {
        f: () => {
          term.writeln(
