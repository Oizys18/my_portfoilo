  ...
  useEffect(() => {
    ...
    const projects = {
      ...
      video: {
        ...
        description: {
          ko: [
            'ffmpeg + HTML5 video player를 이용해 영상을 브라우저에서 구간 크롭 및 저장하는 프로젝트',
            '유저 인풋 영상을 로드, 전체 길이 측정 및 썸네일 생성, 크롭 및 저장 기능',
            '이후 추가 코덱 설치 및 명령어 설정으로 다른 코덱/확장자로 인코딩도 가능하다.'
          ],
          en: [
    ...
    const commands = {
      ...
      platform: {
        f: () => {
          term.writeln(
            [
              `${locale === 'ko' ? '브라우저 최적화' : 'Browser support'}`,
        ...
        description: `${
          locale === 'ko' ? '지원 플랫폼' : 'Supporting Platforms'
        }`
      }
    }
